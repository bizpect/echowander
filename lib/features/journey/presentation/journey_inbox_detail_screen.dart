...
class _JourneyInboxDetailScreenState
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            if (widget.item!.recipientStatus == 'PASSED') ...[
                              ...
                              OutlinedButton(
                                onPressed:
                                    _isActionLoading ? null : _handlePass,
                                child: Text(l10n.inboxPassCta),
                              ),
                              const SizedBox(height: 8),
                              OutlinedButton(
                                onPressed:
                                    _isActionLoading ? null : _handleReport,
                                child: Text(l10n.inboxReportCta),
                              ),
                              const SizedBox(height: 8),
                              OutlinedButton(
                                onPressed:
                                    _isActionLoading ? null : _handleBlock,
  ...
  Future<void> _handleRespond() async {
    ...
    if (kDebugMode) {
      debugPrint(
        '[InboxReplyTrace][UI] _handleRespond - text length: ${text.length}',
      );
    }
    if (text.isEmpty) {
      if (kDebugMode) {
        debugPrint(
          '[InboxReplyTrace][UI] _handleRespond - ABORT: text is empty, showing validation dialog',
        );
      }
    ...
    try {
      ...
      if (kDebugMode) {
        debugPrint(
          '[InboxReplyTrace][UI] _handleRespond - clearing text field',
        );
      }
